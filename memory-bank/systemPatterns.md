...
- **Runtime Architecture**: `aiohttp` web application exposes Telegram webhook at `/process-tg-updates` (POST only) and health at `/health`. Traefik routes only these paths; other requests return 404 at edge. Updates forwarded to shared `aiogram` dispatcher hosted in `src/app/handlers`. Execution wrapped with `logfire` spans for observability and guarded by timeout/error helpers. **Dispatcher errors**: `@dp.errors()` in `handlers/error_handlers.py` logs unexpected failures once, gives private/callback user feedback, swallows expected domain errors (`is_group_inaccessible_error`, `is_permission_error`), re-raises `ModelAPIError` and transient errors for webhook handling. **Webhook HTTP policy (conservative)**: `main.handle_unhandled_exception` returns 503 + `retry: true` only for `is_webhook_retryable` (Telegram network/server, connection-level DB/asyncpg, OSError/ConnectionError); unknown bugs and permanent Telegram/DB errors ack with 200 (no `update_id` idempotency — aggressive retry would double LLM/credits). **Telegram helpers**: `common/telegram_errors.py` centralizes permission/inaccessible/message-not-found classification (type + message markers). **Bot API retries**: `retry_on_network_error` in `utils.py` retries `TelegramNetworkError`, `TelegramServerError`, `TelegramRetryAfter` (honors `retry_after`, capped at 30s) plus aiohttp/OS errors; does not retry `TelegramBadRequest` / `Forbidden` / `NotFound`. Logfire metrics (histograms and gauges) initialized once at module level to ensure proper recording. Handler return values determine logfire span tags - handlers must return descriptive strings to avoid "_ignored" tagging. **Outbound HTTP reuse**: LLM calls share one module-level `ClientSession` + `TCPConnector` in `common/llms.py` (`close_llm_http_resources` on shutdown); `McpHttpClient` holds one session per instance (`close_mcp_http_client` on shutdown) to avoid per-request session allocation.
- **Bot Composition**:
...
- **Spam Decision Flow**: New messages route through filters that skip admins/service messages. **Edited messages** in groups use `filter_handle_edited_message` + the same moderation pipeline when the sender is not **trusted** (see probation below).
...
- **Member probation & trusted skip**: `approved_members.moderation_event_count` tracks moderated events (messages and edits through the full pipeline) after approval. **Trusted** = row exists and `count >= spam.probation_min_events` (default 3). New approvals get `count = 1` on insert; further completed moderations increment until trusted. **Probation** members always run full LLM (including edits); never `message_trusted_member_skipped`. **Trusted** members skip LLM on new messages (lookup-only cache upsert). **Grandfather**: migration `--add-moderation-event-count` sets all existing rows to `probation_min_events` so legacy members keep edit skip (tactic 10 not fixed for them). Admin **Не спам** callback calls `set_moderation_events` to instant trust even if `add_member` no-ops. Ban removes row (counter gone).
- **Message Lookup Cache (PostgreSQL)**: Replaces former Logfire-based lookup. Table `message_lookup_cache` stores metadata (chat_id, message_id, effective_user_id, message_text, reply_to_text) and classification context (stories_context, account_signals_context) for all processed messages. Saved at two points: (A) when **trusted** user message is skipped (`message_trusted_member_skipped`), (B) after full `is_spam` pipeline (including probation edits). When admin forwards a message to add spam example, `find_message_by_text_and_user` looks up in PostgreSQL. If stories/account_signals are empty (trusted-skip case), `collect_user_context` is called on-demand. TTL 7 days, cleaned by scheduled job.
- **User Mode Control**: `/mode` in private chat cycles `moderation_mode`: `notify` → `delete` → `delete_silent` → `notify`. Stored as PostgreSQL enum `moderation_mode` on `administrators` (migration `--add-moderation-mode`; legacy `delete_spam` dropped via `--drop-delete-spam` after deploy). `Administrator.auto_deletes_spam` is true for `delete` and `delete_silent`; `skips_auto_delete_notification` only for `delete_silent`. **Silent auto-delete**: high-confidence spam still deleted/banned, but no per-removal DM; `notify_admins` filters out `delete_silent` admins from auto-delete confirmation DMs. If all admins are `delete_silent`, returns early (no group fallback, no pending spam example). Low-confidence review and permission warnings still notify all admins. **Silent mode has no false-positive undo via forward** — the message is already deleted; do not document forward-to-bot recovery for `delete_silent`. Normal `delete` mode still sends a per-removal DM with **Not spam**. Phase-2 deploy dual-writes `delete_spam` when column still exists.
...
- **MTProto Userbot Notifications**: Promotional notifications to blocked spammers/channel admins are controlled by feature flag `spam.notify_spammers_via_mcp` with env override `SPAM_NOTIFY_SPAMMERS_VIA_MCP` (`1/0`, `true/false`, `yes/no`, `on/off`). Default is **disabled** to avoid unsolicited outbound messages and Telegram penalties. When enabled, behavior is unchanged: on blocked spam with discovered channels, send via MTProto userbot to human spammer or channel admins using existing `channels.getFullChannel` data and `LinkedChannelSummary`.
