...
async def run_scheduled_jobs() -> None:
    ...
    try:
        ...
        logger.warning(f"message_lookup_cache cleanup failed: {e}", exc_info=True)

    try:
        await cleanup_old_message_history(days=ttl["message_history"])
    except Exception as e:
        logger.warning(f"message_history cleanup failed: {e}", exc_info=True)

    try:
        await cleanup_pending_spam_examples(days=ttl["pending_spam"])
    except Exception as e:
        logger.warning(f"pending spam_examples cleanup failed: {e}", exc_info=True)
